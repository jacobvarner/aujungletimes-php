...
class Paginator {
	...
	public function page_links($path = '?', $ext = null) {
		...
		if ($lastpage > 1) {
			...
			if ($this->_page > 1) {
				$pagination .= "<li><a href='" . $path . "$this->_instance=$prev" . "$ext'>&laquo;</a></li>";
			} else {
				$pagination .= "<li class='disabled'><a href='#' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a>";
			}
			...
			if ($lastpage < 7 + ($adjacents * 2)) {
				...
				if ($this->_page < 1 + ($adjacents * 2)) {
					...
					for ($counter = $this->_page - $adjacents; $counter <= $this->_page + $adjacents; $counter++) {
						if ($counter == $this->_page) {
							$pagination .= "<li class='active'><a href='#'>" . $current . "<span class='sr-only'>(current)</span></a></li>";
						} else {
							$pagination .= "<li><a href='" . $path . "$this->_instance=$counter" . "$ext'>$counter</a></li>";
						}
			...
			if ($this->_page < $counter - 1) {
				$pagination .= "<li><a href='" . $path . "$this->_instance=$next" . "$ext'>&raquo</a></li>";
			} else {
				$pagination .= "<li class='disabled'><a href='#' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a>";
			}
